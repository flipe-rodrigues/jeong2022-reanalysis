...
[da_late_snippets,da_late_time] = ...
    signal2eventsnippets(time,da,cs_plus_onset_times,late_period,dt);

%% compute 'DA response' metric

% preallocation
da_early_response = nan(n_rewards,1);
da_late_response = nan(n_rewards,1);
...
sp_baseline = subplot(n_rows,n_cols,3+n_cols*0);
sp_early = subplot(n_rows,n_cols,4+n_cols*0);
sp_late = subplot(n_rows,n_cols,5+n_cols*0);
sp_earlyresponse = subplot(n_rows,n_cols,(3:4)+n_cols*1);
sp_lateresponse = subplot(n_rows,n_cols,(3:4)+n_cols*2);
sp_test9 = subplot(n_rows,n_cols,(3:4)+n_cols*3);
...
xlabel(sp_early,'Time since CS (s)');
ylabel(sp_early,'DA (a.u.)');
xlabel(sp_late,'Time since US (s)');
ylabel(sp_late,'DA (a.u.)');
xlabel(sp_earlyresponse,'Reward #');
ylabel(sp_earlyresponse,'DA CS response (a.u.)');
xlabel(sp_lateresponse,'Reward #');
ylabel(sp_lateresponse,'DA US response (a.u.)');
xlabel(sp_test9,'Reward #');
ylabel(sp_test9,'Normalized DA response');
...
da_early_mu = nanmean(da_early_snippets);
da_early_sig = nanstd(da_early_snippets);
da_early_sem = da_early_sig ./ sqrt(n_rewards);
errorpatch(da_early_time,da_early_mu,da_early_sem,cs_previous_clr,...
    'edgecolor','none',...
    'facealpha',.25,...
    'parent',sp_early);
plot(sp_early,...
...
da_late_mu = nanmean(da_late_snippets);
da_late_sig = nanstd(da_late_snippets);
da_late_sem = da_late_sig ./ sqrt(n_rewards);
errorpatch(da_late_time,da_late_mu,da_late_sem,cs_previous_clr,...
    'edgecolor','none',...
    'facealpha',.25,...
    'parent',sp_late);
plot(sp_late,...
...
% test 9: backpropagation of DA signals during learning
normalization_term = nanmean(...
    da_early_response(end - normalization_window_length + 1 : end));
scatter(sp_test9,...
    reward_idcs,...
    da_early_response/normalization_term,20,...
    'marker','o',...
    'markerfacealpha',.5,...
    'markerfacecolor','k',...
    'markeredgecolor','k',...
...
scatter(sp_test9,...
    reward_idcs,...
    da_late_response/normalization_term,20,...
    'marker','o',...
    'markerfacealpha',.5,...
...
% plot(sp_test9,...
%     reward_idcs./n_rewards,cumsum(da_early_response)/sum(da_early_response),...
%     'color','k',...
%     'linewidth',1.5);
% plot(sp_test9,...
%     [1,1]*cs_extension_idx./n_rewards,ylim(sp_test9),'--k');
% plot(sp_test9,...
%     [0,1],[0,1],'--k');
% text(sp_test9,.25,.75,'decreases',...
%     'color',[1,1,1]*.75,...
%     'horizontalalignment','center',...
%     'units','normalized');
% text(sp_test9,.75,.25,'increases',...
%     'color',[1,1,1]*.75,...
%     'horizontalalignment','center',...
%     'units','normalized');
