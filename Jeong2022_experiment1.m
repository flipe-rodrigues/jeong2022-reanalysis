...
n_rows = 1;
n_cols = 2;
sp_test1 = subplot(n_rows,n_cols,1);
sp_test2 = subplot(n_rows,n_cols,2);
...
[rho,pval] = corr(xx(~nan_flags),da_response(~nan_flags));
mdl = fitlm(xx,da_response);
scatter(sp_test1,xx(sorted_idcs),da_response(sorted_idcs),15,...
    colorlerp([iri_short_clr; iri_long_clr],n_rewards),...
    'marker','o',...
    'markerfacealpha',.5,...
    'markerfacecolor','flat',...
...
[rho,pval] = corr(iri(~nan_flags),da_response(~nan_flags));
mdl = fitlm(iri,da_response);
scatter(sp_test2,iri,da_response,15,...
    colorlerp([early_clr; late_clr],n_rewards),...
    'marker','o',...
    'markerfacealpha',.5,...
    'markerfacecolor','flat',...
    'markeredgecolor','flat',...
    'linewidth',1);
text(sp_test2,.05,.05,'Training stage:',...
    'horizontalalignment','left',...
    'verticalalignment','bottom',...
    'units','normalized',...
    'color','k');
text(sp_test2,.575,.05,'early',...
    'horizontalalignment','left',...
    'verticalalignment','bottom',...
    'units','normalized',...
    'color',stage_clrs(1,:));
text(sp_test2,.775,.05,'late',...
    'horizontalalignment','left',...
    'verticalalignment','bottom',...
    'units','normalized',...
    'color',stage_clrs(end,:));
plot(sp_test2,mdl.Variables.x1,mdl.predict(mdl.Variables.x1),...
    'color',highlight_clr,...
    'linestyle',repmat('-',1,1+(mdl.Coefficients.pValue(end)>.05)),...
    'linewidth',1.5);
...
save_path = 'C:\Users\flipe\Desktop\retreat';
png_file = fullfile(save_path,[fig.Name,'.png']);
