...
%% CS onset times
cs_onset_times = cumsum(icsi) + pre_cs_delay;
...
%% previous & intermediate CS/reward pairs
...
sp_icsi = subplot(n_rows,n_cols,n_cols-1+n_cols*0);
...
set(sp_icsi,...
    'xlim',[0,110]);
set([sp_microstimulus,sp_eligibility],...
    'xlim',[0,40]);
set(sp_reward,...
    'ycolor','none');
...
for ii = 1 : n_stages
    ...
    da_reward_mu = nanmean(da_us_snippets(idcs,:));
...
scatter(sp_test1,xx,da_us_response,20,...
