...
n_rewards = 500;
% should simulate way more, but then only plot the final 500, since they
% look only at asymptotic performance (once anticipatory licking has been
% established) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
%% CS(intermediate)-US(previous) intervals
...
intermediate_flags = icsi < (reward_times - cs_onset_times);
cs = categorical(intermediate_flags,[0,1],{'CS_{prev.}','CS_{interm.}'});
cs_set = categories(cs);

% format shortG
% a = [icsi,cs_onset_times,reward_times,csusi,cs_intermediate_flags];
% a(1:25,:)
...
% specific to baseline-corrected CS responses
da_baseline_snippets = ...
    signal2eventsnippets(time,da,cs_onset_times,baseline_period,dt);
% da_baseline_previous_snippets = signal2eventsnippets(...
%     time,da,cs_onset_times(~intermediate_flags),baseline_period,dt);
% da_baseline_intermediate_snippets = signal2eventsnippets(...
%     time,da,cs_onset_times(intermediate_flags),baseline_period,dt);
% da_cs_previous_snippets = signal2eventsnippets(...
%     time,da,cs_onset_times(~intermediate_flags),cs_period,dt);
% da_cs_intermediate_snippets = signal2eventsnippets(...
%     time,da,cs_onset_times(intermediate_flags),cs_period,dt);
% da_us_previous_snippets = signal2eventsnippets(...
%     time,da,reward_times(~intermediate_flags),us_period,dt);
% da_us_intermediate_snippets = signal2eventsnippets(...
%     time,da,reward_times(intermediate_flags),us_period,dt);
...
ylabel(sp_csresponse,'DA response (a.u.)');
...
stem(sp_stimulus,...
    ...
    'linewidth',1);
stem(sp_stimulus,...
    reward_times(~intermediate_flags),ones(sum(~intermediate_flags),1),...
    'color',cs_previous_clr,...
    'marker','.',...
    'markersize',20,...
    'linewidth',1);
...
scatter(sp_csresponse,...
    reward_idcs(~intermediate_flags),...
...
da_baseline_mu = nanmean(da_baseline_previous_snippets(~intermediate_flags,:));
da_baseline_sig = nanstd(da_baseline_previous_snippets(~intermediate_flags,:));
da_baseline_sem = da_baseline_sig ./ sqrt(sum(~intermediate_flags));
...
plot(sp_baseline,...
    ...
    'linestyle','-',...
    'linewidth',1.5);
da_baseline_mu = nanmean(da_baseline_snippets(intermediate_flags,:));
da_baseline_sig = nanstd(da_baseline_snippets(intermediate_flags,:));
da_baseline_sem = da_baseline_sig ./ sqrt(sum(intermediate_flags));
errorpatch(da_baseline_time,da_baseline_mu,da_baseline_sem,cs_intermediate_clr,...
    'edgecolor','none',...
    'facealpha',.25,...
...
da_cs_mu = nanmean(da_cs_snippets(~intermediate_flags,:));
da_cs_sig = nanstd(da_cs_snippets(~intermediate_flags,:));
da_cs_sem = da_cs_sig ./ sqrt(sum(~intermediate_flags));
...
da_us_mu = nanmean(da_us_snippets(~intermediate_flags,:));
da_us_sig = nanstd(da_us_snippets(~intermediate_flags,:));
da_us_sem = da_us_sig ./ sqrt(sum(~intermediate_flags));
...
bslcorrcs_response_ratio = ...
    nanmean(da_bslcorrectedcs_response(intermediate_flags)) / ...
    nanmean(da_bslcorrectedcs_response(~intermediate_flags));
...
cs_response_ratio = ...
    nanmean(da_cs_response(intermediate_flags)) / ...
    nanmean(da_cs_response(~intermediate_flags));
...
us_response_ratio = ...
    nanmean(da_us_response(intermediate_flags)) / ...
    nanmean(da_us_response(~intermediate_flags));
