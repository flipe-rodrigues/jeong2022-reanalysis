...
us_counts = histcounts(us_times,state_edges);
n_rewards = sum(us_counts);
...
% td lambda
[state_inh,value_inh,rpe_inh] = inhibitiontdlambda(...
    time,[],stimulus_times,us_times,microstimuli,[],rpe_inhibition,...
    'alpha',alpha,...
    'gamma',gamma,...
    'lambda',lambda);

% compute 'DA signal'
padded_rpe_inh = padarray(rpe_inh,dlight_kernel.nbins/2,0);
da_inh = conv(padded_rpe_inh(1:end-1),dlight_kernel.pdf,'valid');
da_inh = da_inh + psi * max(dlight_kernel.pdf);

% get CS- & US-aligned snippets of DA signal
[da_inh_baseline_snippets,~] = signal2eventsnippets(...
    time,da_inh,cs1_onset_times,baseline_period,dt);
[da_inh_cs1_snippets,~] = signal2eventsnippets(...
    time,da_inh,cs1_onset_times,cs1_period,dt);
[da_inh_cs2_snippets,~] = signal2eventsnippets(...
    time,da_inh,cs1_onset_times,cs2_period,dt);
[da_inh_us_snippets,~] = signal2eventsnippets(...
    time,da_inh,cs1_onset_times,us_period,dt);

% preallocation
da_inh_cs1_response = nan(n_rewards,1);
da_inh_cs2_response = nan(n_rewards,1);
da_inh_us_response = nan(n_rewards,1);

% iterate through rewards
for ii = 1 : n_rewards
    
    % compute 'DA response' metrics
    da_inh_cs1_response(ii) = ...
        sum(da_inh_cs1_snippets(ii,:)) - sum(da_inh_baseline_snippets(ii,:));
    da_inh_cs2_response(ii) = ...
        sum(da_inh_cs2_snippets(ii,:)) - sum(da_inh_baseline_snippets(ii,:));
    da_inh_us_response(ii) = ...
...
sp_test10_initialdynamics = subplot(n_rows,n_cols,5+n_cols*2);
sp_test10_asymptoticdynamics = subplot(n_rows,n_cols,5+n_cols*3);
sp_test11_cs1 = subplot(n_rows,n_cols,6+n_cols*2);
sp_test11_b = subplot(n_rows,n_cols,6+n_cols*3);
...
xlabel(sp_test10_backpropagation,'Trial #');
ylabel(sp_test10_backpropagation,{'Normalized DA response',...
    sprintf('(1 = <CS1> last %i trials)',...
...
ylabel(sp_test10_initialdynamics,{'\DeltaNormalized DA response',...
    sprintf('<CS2 - CS1> first %i trials',...
    normalization_window_length)});
xlabel(sp_test10_asymptoticdynamics,...
    sprintf('Trial # (%i-%i)',...
    min(xlim(sp_test10_asymptoticdynamics)),...
    max(xlim(sp_test10_asymptoticdynamics))));
ylabel(sp_test10_asymptoticdynamics,{'Normalized CS2 DA response',...
    sprintf('(1 = <CS1> last %i trials)',...
    normalization_window_length)});
xlabel(sp_test11_cs1,'Trial #');
ylabel(sp_test11_cs1,{'Normalized CS1 DA response',...
    sprintf('(1 = <US> first %i trials)',...
    normalization_window_length)});
...
% test 11:
normalization_term = nanmean(...
    da_us_response(1 : normalization_window_length));
da_norm_cs1_response = da_cs1_response / normalization_term;
scatter(sp_test11_cs1,...
    reward_idcs,da_norm_cs1_response,20,...
    'marker','o',...
    'markerfacealpha',.5,...
    'markerfacecolor','k',...
    ...
    'linewidth',1);
normalization_term = nanmean(...
    da_inh_us_response(1 : normalization_window_length));
da_inh_norm_cs1_response = da_inh_cs1_response / normalization_term;
scatter(sp_test11_cs1,...
    reward_idcs,da_inh_norm_cs1_response,20,...
    'marker','o',...
    'markerfacealpha',.5,...
    'markerfacecolor','r',...
    'markeredgecolor','r',...
...
% update axes
set([sp_baseline,sp_cs1,sp_cs2,sp_us],...
    'ylim',ylim(sp_baseline)+[-1,1]*.05*range(ylim(sp_baseline)));
