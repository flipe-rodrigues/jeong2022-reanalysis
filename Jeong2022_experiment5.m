...
%% concatenate predictable and background reward times
...
%     'gamma',gamma,...
%     'lambda',lambda);
[state,value,rpe,rwdrate,exp5_weights,~] = difftdlambda(...
    time,[],stimulus_times,reward_times,microstimuli,exp3_weights,...
    'alpha',alpha,...
    'gamma',gamma,...
    'lambda',lambda);
...
%% compute training stage indices ('pre' & 'post' adding bg rewards)
...
n_rows = 4;
n_cols = 3;
sp_state = subplot(n_rows,n_cols,2:n_cols-1);
sp_bgiri = subplot(n_rows,n_cols,n_cols);
...
sp_uschange = subplot(n_rows,n_cols,n_cols+n_cols*[2,3]);
...
xlabel(sp_uschange,'Trials relative to introduction of background rewards');
ylabel(sp_uschange,'Cummulative DA response at US');
...
plot(sp_rwdrate,...
    ...
    'linewidth',1);
plot(sp_rwdrate,...
    [1,1]*bg_reward_times(1),ylim(sp_rwdrate),'--k');
...
% test 7b: DA CS responses as a function of trial number
plot(sp_test7,...
    cs_plus_idcs./n_cs_plus,...
    cumsum(da_cs_response)/sum(da_cs_response(cs_plus_idcs<exp_start_idx)),...
    'color','k',...
    'linewidth',1.5);
plot(sp_test7,...
...
% DA US responses as a function of trial number
exp_flags = reward_idcs >= exp_start_idx;
plot(sp_uschange,...
    (exp_start_idx:n_cs_plus)-exp_start_idx,...
    cumsum(da_us_response(exp_flags)),...
    'color','k',...
    'linewidth',1.5);
plot(sp_uschange,...
    [1,1]*exp_start_idx,ylim(sp_uschange),'--k');
plot(sp_uschange,...
    [0,n_cs_plus],[0,0],'--k');
...
% save_path = 'C:\Users\User\Desktop\retreat';
% svg_file = fullfile(save_path,['bg','.svg']);
