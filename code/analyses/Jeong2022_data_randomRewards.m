...
%% analysis parameters
delivery_period = [-1,1] * max(iri_cutoff,3);
...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        lri = arrayfun(@(x)...
            firstlick_times(x) - lick_times(ismember(lick_idcs,x-1)),...
            reward_idcs,...
            'uniformoutput',false);
        ...
        random_idcs = sum(random_times > reward_times',2);
        tri = arrayfun(@(x)...
            firstlick_times(x) - random_times(ismember(random_idcs,x-1)),...
            reward_idcs,...
            'uniformoutput',false);
        ...
        for ii = 1 : n_rewards
            ...
            da_reward_response(ii) = ...
                sum(da_reward_snippets(ii,:) * dt) / range(reward_period) - ...
...
figure(...
    'windowstyle','docked',...
    'numbertitle','off',...
    'name','reaction_times_vs_DA_response',...
    'color','w');
...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        reward_flags = ...
            ...
            session_flags & ...
            valid_flags;
        x = data.rt(reward_flags);
        y = data.da.response(reward_flags);
        X = [ones(size(x)),x];
        betas = robustfit(x,y);
        scatter(sps(mm),...
            x,y,25,...
...
%% plot lick-reward-interval distributions
...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        for rr = 1 : n_rewards
            reward_idx = reward_idcs(rr);
            lri_rewards = data.contingency.tri{reward_idx};
            n_licks = numel(lri_rewards);
            plot(sps(mm),...
                repmat(data.reward.index.mouse(reward_idx),n_licks,1),lri_rewards,'.',...
                'markersize',1,...
                'color','k');
        end
...
%% reward delivery-aligned average lick rate (split by session)

% figure initialization
figure(...
    'windowstyle','docked',...
    'numbertitle','off',...
    'name','licks_delivery_averages',...
    'color','w');
...
%% reward delivery-aligned average lick rate (split by training stage)
...
%% reward collection-aligned average lick rate (split by session)

% figure initialization
figure(...
    'windowstyle','docked',...
    'numbertitle','off',...
    'name','licks_collection_averages',...
    'color','w');
...
%% reward collection-aligned average lick rate (split by training stage)
...
% axes initialization
xxtick = [1,100:100:max(data.reward.index.mouse)];
...
mouse_clrs = gray(n_mice+1);

% sort mice based on total number of rewards
n_rewards_per_mouse = cellfun(@(x) ...
    max(data.reward.index.mouse(data.mouse == x)),mouse_ids);
[~,sorted_idcs] = sort(n_rewards_per_mouse,'descend');
...
% iterate through mice
for mm = 1 : n_mice
    mouse_flags = data.mouse == mouse_ids{sorted_idcs(mm)};
    reward_flags = ...
        valid_flags & ...
        mouse_flags;
    x = data.reward.index.mouse(reward_flags);
    X = [ones(size(x)),x];
    y = nanzscore(data.da.response(reward_flags));
    y = nanzscore(y);
    nan_flags = isnan(y);
    
    [mdl,gof_poly1] = fit(x(~nan_flags),y(~nan_flags),'poly1');
    [mdl_poly2,gof_poly2] = fit(x(~nan_flags),y(~nan_flags),'poly2');
...
for mm = 1 : n_mice
    ...
    reward_flags = ...
        valid_flags & ...
        mouse_flags;
    x = data.iri.nominal(reward_flags);
    X = [ones(size(x)),x];
    y = nanzscore(data.da.response(reward_flags));
    y = nanzscore(y);
    nan_flags = isnan(y);
