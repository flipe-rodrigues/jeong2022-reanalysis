...
trace_dur = 1;
cs_dur_set = [2,8,8,8,8];
us_delay_set = cs_dur_set + trace_dur;

%% background reward settings
bg_min_delay = 6;

%% analysis parameters
trial_period = [0,max(us_delay_set)] + [-1,1] * 20;
reward_period = [-2,2];
baseline_period = [-1,0];
roi_period = [0,2];
...
for mm = 1 : n_mice
    ...
    for ee = 1 : n_subexperiments
        ...
        for ss = 1 : n_sessions
            ...
            iti = cs_times - [0;cs_times(1:end-1)+us_delay_set(ee)];
...
%                 dff = (dff - f0) ./ f0;
            ...
            [da_us_baseline_snippets,~] = signal2eventsnippets(...
                time,da,cs_times+us_delay_set(1),baseline_period,dt,nanify);
            [da_us_roi_snippets,~] = signal2eventsnippets(...
                time,da,cs_times+us_delay_set(1),roi_period,dt,nanify);
...
for mm = 1 : n_mice
    ...
    if want2save
        ...
        print(gcf,png_file,'-dpng','-r600','-painters');
    end
end
...
for mm = 1 : n_mice
    ...
    figure(...
        'windowstyle','docked',...
        'numbertitle','off',...
        'name',sprintf('%s_s_trial_reaction',...
        strrep(mouse_ids{mm},'_',''),experiment_id),...
        'color','w');
...
set(sps,...
    'xlim',reward_period+us_delay_set(1),...
    'xlim',trial_period,...
    'xtick',unique([0,cs_dur_set,us_delay_set]),...
    'ylimspec','tight',...
    'xscale','linear',...
    'nextplot','add',...
    'linewidth',2,...
...
for mm = 1 : n_mice
    ...
    patch(sps(mm),...
        ...
        'linewidth',1.5);
    patch(sps(mm),...
        [roi_period,fliplr(roi_period)]+us_delay_set(1),...
        [-1,-1,1,1]*range(yylim)*.02+yymax,'k',...
        'edgecolor','k',...
        'facealpha',1,...
        'linewidth',1.5);
    ...
    % plot reference lines
    plot(sps(mm),[0,0],ylim(sps(mm)),'--k');
    plot(sps(mm),[1,1]*baseline_period(2)+us_delay_set(1),ylim(sps(mm)),'-k',...
        'linewidth',1);
end
