...
%     ylabel(sps(mm),'Trial # (sorted by reaction time w/in CS)');
    ...
    plot(sps(mm),[0,0]+3,ylim(sps(mm)),'--w');
end
...
for mm = 1 : n_mice
    ...
    % pseudo legend
    xleg = max(xlim(sps(mm)))-.15*range(xlim(sps(mm)));
...
for mm = 1 : n_mice
    ...
    title(sps(mm),sprintf('%s',mouse_ids{mm}),...
        'interpreter','none');
    xlabel(sps(mm),'Time since 1^{st} rewarding lick (s)');
    ylabel(sps(mm),'Reward # (sorted chronologically w/in CS)');
%     ylabel(sps(mm),'Reward # (sorted by reaction time w/in CS)');
    
    % plot DA raster
    da_mat = [...
        data.da.reward_baseline(trial_flags,1:end-1),...
        data.da.reward(trial_flags,:)];
%     sorted_idcs = 1 : n_trials;
    sorting_mat = [data.rt(trial_flags),data.session(trial_flags)];
    ...
    % plot reaction times
    rts = -data.rt(trial_flags);
    session_idcs = data.session(trial_flags);
    scatter(sps(mm),...
        rts(sorted_idcs),1:n_trials,10,...
...
for mm = 1 : n_mice
    ...
    trial_flags = ...
        mouse_flags & ...
        reward_flags;
    x = 1:sum(trial_flags);%data.trial.index.mouse(trial_flags);
