...
n_cs = numel(cs_set);
cs_plus_flags = cs == 'CS+';
n_cs_plus = sum(cs_plus_flags);
cs_plus_idcs = (1 : n_cs_plus)';
...
%% extinction
omission_trial_idx = n_trials / 2;
omission_trial_flags = trial_idcs > omission_trial_idx;

%% reward times
reward_times = click_times + reaction_times;
reward_times = dt * round(reward_times / dt);
reward_times(omission_trial_flags) = nan;
...
%     'gamma',gamma,...
%     'lambda',lambda);
[state,value,rpe,rwdrate,exp4_weights] = difftdlambda(...
    time,[],stimulus_times,reward_times,microstimuli,exp3_weights,...
    'alpha',alpha,...
    'gamma',gamma,...
    'lambda',lambda);
...
% preallocation
da_cs_response = nan(n_cs_plus,1);
da_us_response = nan(n_cs_plus,1);

% iterate through CS+ presentations
for ii = 1 : n_cs_plus
    da_cs_response(ii) = ...
...
sp_test6 = subplot(n_rows,n_cols,n_cols-1+n_cols*[2,3]);
sp_test7 = subplot(n_rows,n_cols,n_cols+n_cols*[2,3]);
...
set([sp_da_mu,sp_da,sp_value_mu,sp_value],...
    'xlim',[-1.5,11.5]);%[-pre_cs_delay,max(trial_dur)+iti_delay]);
set(sp_usresponse,...
    'xlim',[1,n_cs_plus]);
set(sp_test6,...
    'xlim',[0,1],...
...
extinction_time = trial_onset_times(omission_trial_idx);
plot(sp_rwdrate,...
    time,rwdrate,...
    'color','k',...
    'linewidth',1);
plot(sp_rwdrate,...
    [1,1]*extinction_time,ylim(sp_rwdrate),'--k');

% test 6: DA CS responses as a function of trial number
omission_idx = find(omission_trial_flags(cs_plus_flags),1);
plot(sp_test6,...
    cs_plus_idcs./n_cs_plus,cumsum(da_cs_response)/sum(da_cs_response),...
    'color','k',...
    'linewidth',1.5);
plot(sp_test6,...
    [1,1]*omission_idx./n_cs_plus,ylim(sp_test6),'--k');
plot(sp_test6,...
    [0,1],[0,1],'--k');
...
scatter(sp_usresponse,...
    cs_plus_idcs,da_us_response,20,...
    'marker','o',...
    'markerfacealpha',.5,...
    'markerfacecolor','k',...
    'markeredgecolor','k',...
    'linewidth',1);
plot(sp_usresponse,...
    [1,1]*omission_idx,ylim(sp_usresponse),'--k');
plot(sp_usresponse,...
    [0,n_cs_plus],[0,0],'--k');

% test 7a: DA US responses as a function of trial number
plot(sp_test7,...
    cs_plus_idcs./n_cs_plus,...
    cumsum(da_cs_response)/sum(da_cs_response(cs_plus_idcs<omission_idx)),...
    'color','k',...
    'linewidth',1.5);
plot(sp_test7,...
    [1,1]*.5,ylim(sp_test7),'--k');
plot(sp_test7,...
    [0,1],[1,1],'--k');
