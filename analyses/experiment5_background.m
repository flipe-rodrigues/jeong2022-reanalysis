...
%% analysis parameters
bg_min_delay = 6;
trial_period = [0,csus_delay] + [-2,3] * bg_min_delay;
baseline_period = [-1,0];
roi_period = [0,2];
...
%     ylabel(sps(mm),'Trial # (sorted by reaction time w/in CS)');
    
    % plot DA raster
    da_mat = data.da.trial(trial_flags,:);
    sorting_mat = [double(data.trial.cs(trial_flags)),data.rt(trial_flags)];
    [~,sorted_idcs] = sortrows(sorting_mat,[1]);
    imagesc(sps(mm),trial_period,[],...
        da_mat(sorted_idcs,:),quantile(da_mat,[.001,.999],'all')');
    
    % iterate through CS types
    counter = 0;
    cs_types = unique(data.trial.cs(mouse_flags));
    n_cs_types = numel(cs_types);
    for cc = 1 : n_cs_types
        cs_flags = data.trial.cs == cs_types(cc);
    ...
    plot(sps(mm),xlim(sps(mm)),...
        [1,1]*(sum(data.trial.cs(mouse_flags)=='CS1')+.5),...
        'color',[1,1,1],...
        'linewidth',1.5);
...
%% background reward delivery-aligned DA rasters
...
for mm = 1 : n_mice
    ...
    ylabel(sps(mm),'Background reward #');
    
    % plot DA raster
    da_mat = vertcat(data.da.background{trial_flags,:});
    imagesc(sps(mm),...
        [baseline_period(1),roi_period(2)],[],...
        da_mat,quantile(da_mat,[.001,.999],'all')');
...
%% background reward delivery-aligned average DA (split by session)
...
for mm = 1 : n_mice
    ...
    title(sps(mm),sprintf('%s',mouse_ids{mm}),...
        'interpreter','none');
    xlabel(sps(mm),'Time since background reward (s)');
    ylabel(sps(mm),'DA (\DeltaF/F)');
    ...
    for ss = 1 : n_sessions
        ...
        da_mu = nanmean(da_mat,1);
        da_std = nanstd(da_mat,0,1);
        da_sem = da_std ./ sqrt(sum(~isnan(da_mat)));
        nan_flags = isnan(da_mu) | isnan(isnan(da_sem)) | da_sem == 0;
        xpatch = [da_bg_time(~nan_flags),...
            fliplr(da_bg_time(~nan_flags))];
        ypatch = [da_mu(~nan_flags)-da_sem(~nan_flags),...
            fliplr(da_mu(~nan_flags)+da_sem(~nan_flags))];
        patch(sps(mm),...
            xpatch,ypatch,session_clrs(ss,:),...
            ...
            'facealpha',.25);
        plot(sps(mm),...
            da_bg_time(~nan_flags),da_mu(~nan_flags),'-',...
            'color',session_clrs(ss,:),...
            'linewidth',1);
    end
