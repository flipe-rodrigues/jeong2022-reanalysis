...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        % iterate through trials
        for ii = 1 : n_trials
            reward_idx = find(lick_flags & ...
                event_times >= us_times(ii),1,'first');
            if ~isempty(reward_idx)
                reward_times(ii) = event_times(reward_idx);
            end
        end
        ...
        [da_cs_snippets,da_cs_time] = signal2eventsnippets(...
            time,da,cs_times,reward_period,dt);
...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        for rr = 1 : n_rewards
            ...
            ili_trials = data.ili{reward_idx};
            n_licks = numel(ili_trials);
            plot(sps(mm),...
                repmat(data.trial.index.mouse(reward_idx),n_licks,1),ili_trials,'.',...
                'markersize',1,...
                'color','k');
        end
...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        betas = robustfit(x,y);
        plot(sps(mm),...
            data.trial.index.mouse(trial_flags),...
            data.da.cs_response(trial_flags),'.',...
            'markersize',10,...
            'color',[1,1,1]*.85);
        plot(sps(mm),...
            x,y,'.',...
...
for mm = 1 : n_mice
    ...
    for ss = 1 : n_sessions
        ...
        plot(sps(mm),...
            data.trial.index.mouse(trial_flags),...
            data.da.reward_response(trial_flags),'.',...
            'markersize',10,...
            'color',[1,1,1]*.85);
        plot(sps(mm),...
            x,y,'.',...
